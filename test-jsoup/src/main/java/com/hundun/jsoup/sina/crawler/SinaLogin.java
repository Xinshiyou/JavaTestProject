...
public class SinaLogin {
	...
	private static final Log logger = LogFactory.getLog(SinaLogin.class);
	...
	public HttpClient logonAndValidate(String userName, String pwd) throws HttpException, IOException {
		...
		String initPageURL = "http://login.sina.com.cn/sso/prelogin.php?entry=sso&"
				+ "callback=sinaSSOController.preloginCallBack&su=" + su + "&rsakt=mod&client=ssologin.js(v1.4.5)"
				+ "&_=" + getServerTime();
		...
		String sp = "";
		try {
			sp = new RSAUtil().rsaCrypt(SINA_PK, "10001", pwdString);
		...
		// logger.info("sp=" + sp);
		// logger.info("su=" + su + "&servertime=" + servertime + "&nonce="
		// + nonce + "&sp=" + sp);
		postMethod2Logon.setRequestBody(new NameValuePair[] { new NameValuePair("entry", "weibo"),
				new NameValuePair("gateway", "1"), new NameValuePair("from", ""), new NameValuePair("savestate", "7"),
				new NameValuePair("useticket", "1"), new NameValuePair("ssosimplelogin", "1"),
				new NameValuePair("useticket", "1"), new NameValuePair("vsnf", "1"), new NameValuePair("vsnval", ""),
				new NameValuePair("su", su), new NameValuePair("service", "miniblog"),
				new NameValuePair("servertime", servertime + ""), new NameValuePair("nonce", nonce),
				new NameValuePair("pwencode", "rsa2"), new NameValuePair("rsakv", rsakv), new NameValuePair("sp", sp),
				new NameValuePair("encoding", "UTF-8"), new NameValuePair("prelt", "115"),
