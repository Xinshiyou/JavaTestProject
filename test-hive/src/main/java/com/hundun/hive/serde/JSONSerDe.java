...
public class JSONSerDe implements SerDe {
	...
	public Object deserialize(Writable blob) throws SerDeException {
		...
		try {
			...
			//// Version-1
			// root = UserEvent.convertBean(mapper.readValue(blob.toString(),
			//// UserEvent[].class)[0]);

			//// Version-2
			JsonObject jobj = new JsonParser().parse(blob.toString()).getAsJsonArray().get(0).getAsJsonObject();
			root = new HashMap<String,Object>();
			for (Entry<String, JsonElement> item : jobj.entrySet())
				root.put(item.getKey(), item.getValue().getAsString());
		} catch (Exception e) {
	...
	private Object parseStruct(Object field, StructTypeInfo fieldTypeInfo) {
		 ...
		 * ArrayList<TypeInfo> structTypes =
		 * fieldTypeInfo.getAllStructFieldTypeInfos(); ArrayList<String>
		 * structNames = fieldTypeInfo.getAllStructFieldNames();
		 * 
		 * List<Object> structRow = new ArrayList<Object>(structTypes.size());
		 * if (map != null) { for (int i = 0; i < structNames.size(); i++) {
		 * structRow.add(parseField(map.get(structNames.get(i)),
		 * structTypes.get(i))); } } return structRow;
		 */
	}
	...
	private Object parseList(Object field, ListTypeInfo fieldTypeInfo) {
		...
		return field.toString();
		/*
		 * ArrayList<Object> list = (ArrayList<Object>) field; TypeInfo
		 * elemTypeInfo = fieldTypeInfo.getListElementTypeInfo(); if (list !=
		 * null) { for (int i = 0; i < list.size(); i++) { list.set(i,
		 * parseField(list.get(i), elemTypeInfo)); } } return list.toArray();
		 */
	}
	...
	private Object parseMap(Object field, MapTypeInfo fieldTypeInfo) {
		...
		return field.toString();
		/*
		 * Map<Object,Object> map = (Map<Object,Object>) field; TypeInfo
		 * valueTypeInfo = fieldTypeInfo.getMapValueTypeInfo(); if (map != null)
	 ...
	 */
	private Object deparseStruct(Object obj, StructObjectInspector structOI, boolean isRow) {
